...
module.exports = (bot) => {
  bot.on('interactionCreate', async (interactionParam) => {
    const interaction = interactionParam;
    if (interaction.type === InteractionType.ApplicationCommand) {
      // Проверка на слэш команду
      const argsF = {
      ...
      if (interaction.options._subcommand)
        argsF.subcommand = interaction.options._subcommand; // Если это sub группа - добавить в аргумент
      interaction.options._hoistedOptions.forEach((it) => {
        argsF[it.name] = it.value;
      }); // Добавить опции в аргументы
      ...
      if (CMD)
        CMD(bot, interaction, args, argsF) // Если есть команда - вызвать её.
